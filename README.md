...
> **Note:** The ZIP file contains the complete project files required to run StudySmart. The GitHub repository provides the same project with its version history, Unit test cases, CI and development structure. For a quick demonstration, the ZIP version can be used directly without cloning the repository and set-up.
